...
def main():
    ...
    parser.add_argument("--model_file", default=None, type=str, help="Path to trained LightGCN checkpoint (.pth). Defaults to latest LightGCN file in ./check_point.")
    parser.add_argument("--popularity_blend", default=DEFAULT_POPULARITY_BLEND, type=float, help='Weight for blending popularity scores with model predictions (0 disables blending).')
    parser.add_argument("--seed", default=42, type=int)
    args = parser.parse_args()

    set_seed(args.seed)

    popularity_blend = max(0.0, min(1.0, args.popularity_blend))
    ...
    if args.model_file:
        ...
        if model_path is None:
            raise FileNotFoundError(
                "No checkpoint found. Provide --model_file pointing to a trained LightGCN .pth file."
            )
    ...
    for uid in tqdm(users, desc=f"Predicting with {MODEL_NAME}"):
        if str(uid) in dataset.field2token_id['user_idx']:
            ...
            predicted_item_list = None
            topk_scores = None
            history = None
            if hasattr(test_data, 'uid2history_item'):
                try:
                    history = test_data.uid2history_item[recbole_id]
