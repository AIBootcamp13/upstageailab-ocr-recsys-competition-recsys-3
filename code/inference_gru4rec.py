...
def main():
    ...
    parser.add_argument("--model_file", default=None, type=str, help="Path to trained GRU4Rec checkpoint (.pth). Defaults to latest GRU4Rec file in ./check_point.")
    ...
    if args.model_file:
        ...
        if model_path is None:
            raise FileNotFoundError(
                "No checkpoint found. Provide --model_file pointing to a trained GRU4Rec .pth file."
            )
    ...
    for uid in tqdm(users.keys(), desc=f"Predicting with {MODEL_NAME}", total=len(users), dynamic_ncols=True):
